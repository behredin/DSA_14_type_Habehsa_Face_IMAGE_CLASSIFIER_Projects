...
if authenticate_user():
    ...
    if(selected=='Classification'):
        def load_model():
            model = tf.keras.models.load_model("F:/New chapter/PG/DSA/EMOTION/bmanimage/image_model.hdf5")
            return model
        ...
        if uploaded_file is not None:
            ...
            if Genrate_pred:
                ...
                st.title("Ohhhh...This Guy  Looks Like  {}".format(map_dict [prediction]))
    if(selected=="About"):
        st.markdown("""
        <style>
        .paragraph{
        ...
        """,unsafe_allow_html=True)
        st.markdown('<p class="paragraph">One of our goal is Creating Multiple Image Classification App that classifies a given image. </p> ',unsafe_allow_html=True)
        st.markdown("""
        <style>
